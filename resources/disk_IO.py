...
# Internally all string in the data model are str. They will be encoded to
...
def fs_write_catfile(categories_file, categories, launchers, update_timestamp=0.0):
    ...
    try:
        ...
        for categoryID in sorted(categories, key=lambda x: categories[x]['m_name']):
            ...
            # XML_text() returns str strings that will be encoded to UTF-8 later.
        ...
        if category_element.tag == 'control':
            ...
            for category_child in category_element:
                ...
                if xml_tag == 'finished':
                    ...
                    # Internal data is always stored as str. ElementTree already outputs str.
...
# -------------------------------------------------------------------------------------------------
# Generic file writer
# str_list is a list of str strings that will be joined and written to a file encoded in UTF-8.
# -------------------------------------------------------------------------------------------------
    ...
    for root_element in xml_root:
        ...
        if root_element.tag == 'control':
            for control_child in root_element:
                if control_child.tag == 'update_timestamp':
                    update_timestamp = float(control_child.text)  # Convert str to float
...
def fs_write_VCategory_XML(roms_xml_file, roms):
    ...
    try:
        ...
        str_list.append('<?xml version="1.0" encoding="utf-8" standalone="yes"?>\n')
        str_list.append(
            '<advanced_emulator_launcher_Virtual_Category_index version="{0}">\n'.format(AEL_STORAGE_FORMAT))
...
# Both argument filenames must be str strings.
            ...
            for launcher_element in root_element:
                ...
                for launcher_child in launcher_element:
                    ...
                    if launcher_child.tag != 'roms':
                        ...
                        # >> Some fields have different dictionary key and XML tag!!!
                        # >> Translate accordingly
                        if launcher_child.tag == 'platform':
                            launcher_child.tag = 'gamesys'
                        elif launcher_child.tag == 'publisher':
                            launcher_child.tag = 'studio'
                        elif launcher_child.tag == 'launcherplot':
                            launcher_child.tag = 'plot'
                        ...
                        for rom_element in roms_element:
                            ...
                            for rom_child in rom_element:
                                ...
                                # >> Some fields have different dictionary key and XML tag!!!
                                # >> Translate accordingly
                                if rom_child.tag == 'publisher':
                                    rom_child.tag = 'studio'
                                elif rom_child.tag == 'gameplot':
                                    rom_child.tag = 'plot'
...
def fs_export_ROM_NFO(rom, verbose=True):
    ...
    try:
        ...
        usock.write(full_string.decode("utf-8"))
        usock.close()
    except:
        if verbose:
...
# About reading files in str http://stackoverflow.com/questions/147741/character-reading-from-file-in-python
#
def fs_import_ROM_NFO(roms, romID, verbose=True):
    ...
    if os.path.isfile(nfo_file_path):
        ...
        # >> We assume NFO files are UTF-8. Decode data to str.
...
def fs_export_launcher_NFO(nfo_FileName, launcher):
    ...
    try:
        ...
        f.write(full_string.decode("utf-8"))
        f.close()
    except:
